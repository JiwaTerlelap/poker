...
if (isset($_POST['saveproses']))
{
    ...
    else
    {
        ...
        $qr0 = mysqli_query(fOpenConn(), "SELECT * FROM msgames WHERE gm_id = '$permainan'");
        $rs0 = mysqli_fetch_object($qr0);

        $email_message = "Yth " .$namabank;
        $email_message .= "<br><br>Terima kasih telah mendaftar di BandarBola855";
        $email_message .= "<br>Berikut kami informasi UserID dan Password Anda untuk permainan ( <b>".$rs0->gm_nama."</b> ) : ";
        $email_message .= "<br> - Link Login : <a href='".$rs0->gm_link."'>".$rs0->gm_link."</a>";
    ...
    <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-list"></i> List Member Baru</h2>
                </div>
                <div class="box-content">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        <?php
                        $qr1 = mysqli_query(fOpenConn(), "SELECT * FROM msmember, msgames WHERE mbr_games = gm_id AND mbr_status = 'Baru' ORDER BY mbr_createtm");
                        while ($rs1 = mysqli_fetch_object($qr1))
                        {
                            $createtm = date("d-m-y H:i", strtotime($rs1->mbr_createtm));
                            echo "<tr><td><a style='text-decoration: none;' href='msmember_detail.php?id=$rs1->mbr_id'>$rs1->mbr_email</a></td>";
...
<?php
$qr0 = mysqli_query(fOpenConn(), "SELECT * FROM msmember, msgames WHERE mbr_games = gm_id AND mbr_status = 'Setuju'  $addquery");
...
<div class="row">
    <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-list"></i> List Member</h2>
                </div>
                <div class="box-content">
                    <div class="box row">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        <?php
                        $qr1 = mysqli_query(fOpenConn(), "SELECT * FROM msmember, msgames WHERE mbr_games = gm_id AND mbr_status = 'Setuju'  $addquery ORDER BY mbr_email LIMIT $start, $per_hal");
                        while ($rs1 = mysqli_fetch_object($qr1))
                        {
                            if ($fix_userjabatan == "Master")
                            {
                                $emailshow = $rs1->mbr_email;
                                $hpshow = $rs1->mbr_hp;
                            }
                            else
                            {
                                $emailshow = substr($rs1->mbr_email,0,7)."*****";
                                $hpshow = "********".substr($rs1->mbr_hp,-4);
                            }
                            echo "<tr><td><a style='text-decoration: none;' href='msmember_update.php?id=$rs1->mbr_id'>$emailshow</a></td>";
                            echo "<td>$hpshow</td>";
                            echo "<td>$rs1->mbr_namabank</td>";
                            echo "<td>$rs1->mbr_namarek ( $rs1->mbr_norek )</td>";
                            echo "<td>$rs1->gm_nama</td>";
                            echo "<td>$rs1->mbr_gameid</td></tr>";
                        }
                        ?>
                        </tbody>
                    </table>
