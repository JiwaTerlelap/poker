    ...
    <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-list"></i> List Withdraw Baru</h2>
                </div>
                <div class="box-content">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        <?php
                        $qr1 = mysqli_query(fOpenConn(), "SELECT * FROM msmember, mswithdraw, msgames WHERE mbr_games = gm_id AND wed_status = 'Baru' AND wed_member = mbr_id ORDER BY wed_createtm");
                        while ($rs1 = mysqli_fetch_object($qr1))
                        {
                            $createtm = date("d-m-y H:i", strtotime($rs1->mbr_createtm));
                            echo "<tr><td><a style='text-decoration: none;' href='mswithdraw_detail.php?id=$rs1->mbr_id'>$rs1->mbr_gameid ( $rs1->gm_nama )</a></td>";
                            echo "<td>$rs1->mbr_hp</td>";
                            echo "<td>$rs1->mbr_namabank - $rs1->mbr_namarek ( $rs1->mbr_norek )</td>";
                            echo "<td>$createtm</td>";
                            echo "<td>$rs1->wed_jumlah</td></tr>";
                        }
                        ?>
                        </tbody>
                    </table>
...
-->
<?php
$qr0 = mysqli_query(fOpenConn(), "SELECT * FROM msmember, mswithdraw, msgames WHERE mbr_games = gm_id AND wed_status = 'Setuju' AND wed_member = mbr_id  $addquery");
...
<div class="row">
    <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-list"></i> List Withdraw</h2>
                </div>
                <div class="box-content">
                    <table class="table table-bordered">
                        <thead>
                        ...
                        <tbody>
                        <?php
                        $qr1 = mysqli_query(fOpenConn(), "SELECT * FROM msmember, mswithdraw, msgames WHERE mbr_games = gm_id AND wed_status = 'Setuju' AND wed_member = mbr_id ORDER BY wed_createtm");
                        while ($rs1 = mysqli_fetch_object($qr1))
                        {
                            $createtm = date("d-m-y H:i", strtotime($rs1->mbr_createtm));
